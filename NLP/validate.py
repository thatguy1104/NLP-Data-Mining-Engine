...
matchedPublicationsSDG_filepath = "../NLP/matchedScopusSDG.json"
num_of_sdgs = 18

def read_module_sdg_training_data():
...
def read_module_sdg_count_data():
...
def squashing_function(vec_A, vec_B):
    return 1

def compute_similarity(vec_A, vec_B):
    return cosine_similarity(vec_A, vec_B) * squashing_function(vec_A, vec_B)

def validate():
    count_data = read_module_sdg_count_data()
    training_data = read_module_sdg_training_data()
