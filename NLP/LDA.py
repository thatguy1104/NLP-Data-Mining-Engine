...
class LDA():
    def __init__(self, data, keywords):
        # module-catalogue data frame with columns {ModuleID, Description}.
        self.data = data
    ...
    def create_vectorizer(self, min_n_gram, max_n_gram, min_df, max_df):
        # lemmatize stopwords.
        stopwords = [text_lemmatizer(s) for s in get_stopwords()]
    ...
    def create_model(self, corpus, id2word, eta, n_passes, n_iterations, alpha):
    ...
    def train(self, n_passes, n_iterations, num_top_words):
        ...
        self.writeResults(corpus, num_top_words)

        # Process results, stored in ModelResults directory
        self.processResults(threshold=15)
